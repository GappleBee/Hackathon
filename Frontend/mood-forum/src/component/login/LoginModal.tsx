  ...
  return (
    <form onSubmit={handleSubmit(onLogin)} className="relative flex flex-col rounded-[8px] items-center p-[60px] gap-[24px] self-stretch border-solid w-[535px] h-auto bg-white">
      ...
      <div className="w-[415px] flex flex-col justify-between">
        ...
        <span className="text-[14px] font-[600] leading-[24px] font-poppins text-red-500">{ errors.email_username && (<span>{ errors.email_username.message }</span>) }</span>
      </div>
      <div className="w-[415px] flex flex-col justify-between">
        <span className="text-[14px] font-[600] leading-[24px] font-poppins">Password</span><br/>
        <span className="flex w-[415px] h-[56px]">
          <input {...register("password", passwordValidate)} className="bg-[#D9D9D9] text-[#575A5C] w-full outline-none rounded-tl-[8px] rounded-bl-[8px] py-[16px] pl-[19px]" type="text" placeholder="Enter your password"/>
          <span className="w-[76px] flex justify-center items-center rounded-tr-[8px] rounded-br-[8px] bg-[#D9D9D9]">{basilEye}</span>
        </span>
        <span className="text-[14px] font-[600] leading-[24px] font-poppins text-red-500">{ errors.password && (<span>{ errors.password.message }</span>) }</span>
      </div>
